...
namespace ShaderGraphEssentials
{
    ...
    class NoiseNode : AbstractMaterialNode, IGeneratesBodyCode, IGeneratesFunction, IMayRequireMeshUV
    {
        ...
        const string kPeriodeSlotName = "Periode";
        ...
        private void UpdateSlots()
        {
            ...
            if (IsNoiseSupported())
            {
                ...
                {
                    AddSlot(new Vector1MaterialSlot(PeriodeSlotId, kPeriodeSlotName, kPeriodeSlotName, SlotType.Input, 1.0f));
                    idList.Add(PeriodeSlotId);
                }
        ...
        public void GenerateNodeCode(ShaderStringBuilder sb, GraphContext graphContext, GenerationMode generationMode)
        {
            ...
            if (isNoiseSupported)
            {
                if (m_noisePeriodicity == NoisePeriodicity.Periodic)
                {
                    string periodeValue = GetSlotValue(PeriodeSlotId, generationMode);
                    functionCall += ", " + periodeValue;
                }
        ...
        public void GenerateNodeFunction(FunctionRegistry registry, GraphContext graphContext, GenerationMode generationMode)
        {
            ...
            registry.ProvideFunction(functionName, s =>
            {
                ...
                if (isNoiseSupported)
                {
                    if (m_noisePeriodicity == NoisePeriodicity.Periodic)
                    {
                        var periodeSlot = FindInputSlot<MaterialSlot>(PeriodeSlotId);

                        if (periodeSlot == null)
                            throw new NullReferenceException("periodeSlot null, how is it possible ?");

                        s.Append(", {0} {1}",
                            periodeSlot.concreteValueType.ToShaderString(),
                            kPeriodeSlotName);
                    }
                    if (m_noiseCombine == NoiseCombine.Fractal || m_noiseCombine == NoiseCombine.Turbulence || m_noiseCombine == NoiseCombine.Ridge)
                    {
                ...
                using (s.BlockScope())
                {
                    if (isNoiseSupported)
                    {
                        ...
                        if (m_noisePeriodicity == NoisePeriodicity.Periodic)
                        {
                            s.Append(", pow(2, floor({0}))", kPeriodeSlotName);
                        }
