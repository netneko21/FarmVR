#include "Assets/Plugins/ShaderGraphEssentials/Plugin/Editor/Plugin_LWRP/Shaders/SGE_CustomLightingInternal.hlsl"
...
half3 SGE_SpecularLightingCustom(Light light, half3 viewDir, SGECustomLightingData data)
{
    ...
    half3 halfVec = SafeNormalize(light.direction + viewDir);
    half NdotH = saturate(dot(data.normal, halfVec));
    half modifier = pow(NdotH * lightIntensity, data.smoothness);
    half modifierSmooth = smoothstep(EPSILON * 0.5, EPSILON, modifier);
