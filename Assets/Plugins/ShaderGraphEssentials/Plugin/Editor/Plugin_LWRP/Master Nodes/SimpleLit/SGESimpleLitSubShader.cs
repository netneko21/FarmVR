...
{
    [Serializable]
    class LightWeightSimpleLitSubShader : ISGESimpleLitSubShader
    {
        ...
        private const string PassPath = "SGE_LightweightSimpleLitPass.template";
        private const string ExtraPassPath = "SGE_LightweightSimpleLitExtraPasses.template";
        private const string PBR2DPath = "SGE_Lightweight2DSimpleLitPass.template";
        ...
        Pass m_SimpleLitPass = new Pass
        {
            Name = "SimpleLitForward",
        ...
        public string GetSubshader(IMasterNode masterNode, GenerationMode mode, List<string> sourceAssetDependencyPaths = null)
        {
            ...
            var pass = m_SimpleLitPass;
            var subShader = new ShaderStringBuilder();
            subShader.AppendLine("SubShader");
            using (subShader.BlockScope())
            {
                var materialTags = simpleLitMasterNode.GetMaterialTags();
                ...
                var materialOptions = simpleLitMasterNode.GetMaterialOptions();
            ...
            if (simpleLitMasterNode.CustomEditor != null && !simpleLitMasterNode.CustomEditor.Equals(String.Empty))
                subShader.AppendLine(@"CustomEditor ""{0}""", simpleLitMasterNode.CustomEditor);
        ...
        static string GetShaderPassFromTemplate(string template, SGESimpleLitMasterNode masterNode, Pass pass, GenerationMode mode, SurfaceMaterialOptions materialOptions)
        {
            // ----------------------------------------------------- //
            //                         SETUP                         //
            // ----------------------------------------------------- //
            ...
            bool recomputePositionAfterVertexTransform =
                masterNode.IsSlotConnected(SGESimpleLitMasterNode.PositionSlotId) && masterNode.UpdateVertexPosition.isOn;
