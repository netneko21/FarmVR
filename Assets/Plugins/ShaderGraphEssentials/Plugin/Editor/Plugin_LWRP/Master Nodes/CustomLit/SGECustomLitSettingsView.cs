...
namespace ShaderGraphEssentials
{
    class SGECustomLitSettingsView : VisualElement
    {
        ...
        public SGECustomLitSettingsView(SGECustomLitMasterNode node)
        {
            ...
            ps.Add(new PropertyRow(new Label("Update Vertex position")), (row) =>
            {
                row.Add(new Toggle(""), (field) =>
                {
                    field.value = m_Node.UpdateVertexPosition.isOn;
                    field.RegisterValueChangedCallback(ChangeUpdateVertexPosition);
                });
            });
        ...
        void ChangeUpdateVertexPosition(ChangeEvent<bool> evt)
        {
            if (Equals(m_Node.UpdateVertexPosition, evt.newValue))
                return;

            m_Node.owner.owner.RegisterCompleteObjectUndo("Vertex Output Change");
            ToggleData td = m_Node.UpdateVertexPosition;
            td.isOn = evt.newValue;
            m_Node.UpdateVertexPosition = td;
        }
    }
}